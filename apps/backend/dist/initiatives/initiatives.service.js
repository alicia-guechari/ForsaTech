"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.InitiativesService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const initiative_entity_1 = require("../entities/initiative.entity");
const initiative_support_entity_1 = require("../entities/initiative-support.entity");
let InitiativesService = class InitiativesService {
    initiativeRepository;
    supportRepository;
    constructor(initiativeRepository, supportRepository) {
        this.initiativeRepository = initiativeRepository;
        this.supportRepository = supportRepository;
    }
    async findAll(wilayaId) {
        const query = this.initiativeRepository.createQueryBuilder('init')
            .leftJoinAndSelect('init.user', 'user')
            .leftJoinAndSelect('init.wilaya', 'wilaya')
            .orderBy('init.createdAt', 'DESC');
        if (wilayaId) {
            query.andWhere('wilaya.id = :wilayaId', { wilayaId });
        }
        return query.getMany();
    }
    async findOne(id) {
        return this.initiativeRepository.findOne({
            where: { id },
            relations: { user: true, wilaya: true, supports: true, comments: true },
        });
    }
    async create(user, data) {
        const initiative = this.initiativeRepository.create({
            ...data,
            user,
        });
        return this.initiativeRepository.save(initiative);
    }
    async support(user, initiativeId) {
        const initiative = await this.initiativeRepository.findOneBy({ id: initiativeId });
        if (!initiative)
            throw new common_1.BadRequestException('Initiative not found');
        const existingSupport = await this.supportRepository.findOneBy({
            user: { id: user.id },
            initiative: { id: initiativeId },
        });
        if (existingSupport) {
            throw new common_1.BadRequestException('You already supported this initiative');
        }
        const support = this.supportRepository.create({ user, initiative });
        await this.supportRepository.save(support);
        await this.initiativeRepository.increment({ id: initiativeId }, 'supportCount', 1);
        const updated = await this.initiativeRepository.findOneBy({ id: initiativeId });
        if (updated && updated.supportCount >= updated.threshold && updated.status === initiative_entity_1.InitiativeStatus.PENDING) {
            console.log(`Initiative ${initiativeId} reached threshold! Notifying ODEJ...`);
        }
        return updated;
    }
};
exports.InitiativesService = InitiativesService;
exports.InitiativesService = InitiativesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(initiative_entity_1.Initiative)),
    __param(1, (0, typeorm_1.InjectRepository)(initiative_support_entity_1.InitiativeSupport)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository])
], InitiativesService);
//# sourceMappingURL=initiatives.service.js.map